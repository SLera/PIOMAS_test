...
REGION = [57.5, 67, -60, -20]

IrmingerLabradorSea_mask = np.zeros((720, 720))
    ...
    for j in range(720):
        if (lats[i, j] > REGION[0] and lats[i, j] < REGION[1]):
            if (lons[i, j] > REGION[2] and lons[i, j] < REGION[3]):
                print lons[i, j]
                IrmingerLabradorSea_mask[i, j] = 1
                print IrmingerLabradorSea_mask[i, j]

IrmingerLabradorSea_mask.dump('IrmingerLabrador_mask_Vi_EASE2N.npy')
...
plt.figure()
m.imshow(IrmingerLabradorSea_mask[200:500, 200:500], origin='upper')
m.drawcoastlines()
plt.show()
